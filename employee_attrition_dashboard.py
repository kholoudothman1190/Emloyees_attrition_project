...
with tabs[5]:
    ...
    with col1:
        ...
        hires_by_year = filtered_df["hiring_year"].value_counts().to_frame(name="Count")
        hires_by_year.index.name = "hiring_year"
        hires_by_year = hires_by_year.reset_index()
        hires_by_year = hires_by_year.sort_values("hiring_year")
        st.subheader("Hires by Year")
        st.plotly_chart(px.bar(hires_by_year,x="hiring_year",y="Count",
                color_discrete_sequence=[color_palette[2]]),use_container_width=True)
    with col2:
